...
##Comparison graphs
##Epslion = .25
setwd("C:\\Users\\m_pis\\Box Sync\\Python Examples (ACS)")

dfpriv = read.csv(paste(getwd(),"\\PrivBayes\\DataSynthesizer\\out\\correlated_attribute_mode\\privBayes_E0_0.csv",sep=""))[,-1]

dfpriv = as.data.frame(table(dfpriv))$Freq/samp.size


dflcm = read.csv("C:\\Users\\m_pis\\Box Sync\\Summer Project at Duke (Pistner)\\Master Code Files\\results\\BayesLCM_Full_E0_25_1.csv")[,-1]
dflcm = colMeans(dflcm)

dfgm = read.csv(paste(getwd(),"\\Graphical Models Example\\GM_0_25_1.csv",sep=""),header=FALSE)/samp.size
dfgm = unlist(dfgm)
...
ggplot(data=data.map,aes(x=True, y=Syn, group=Method,color=Method)) +
  ...
  geom_abline(slope=1, intercept=0) + xlab("True Probability") + ylab("Estimated Probability") + theme(axis.text=element_text(size=16),
                                                                                                                 axis.title=element_text(size=16),legend.text=element_text(size=16),legend.title=element_text(size=16))

ggsave(paste(results.dir,"compareE0_25.pdf",sep="/"))

##Epsilon = .50

dfpriv = read.csv(paste(getwd(),"\\PrivBayes\\DataSynthesizer\\out\\correlated_attribute_mode\\privBayes_E1_0.csv",sep=""))[,-1]

dfpriv = as.data.frame(table(dfpriv))$Freq/samp.size


dflcm = read.csv("C:\\Users\\m_pis\\Box Sync\\Summer Project at Duke (Pistner)\\Master Code Files\\results\\BayesLCM_Full_E0_50_2.csv")[,-1]
dflcm = colMeans(dflcm)

dfgm = read.csv(paste(getwd(),"\\Graphical Models Example\\GM_0_50_1.csv",sep=""),header=FALSE)/samp.size
dfgm = unlist(dfgm)
...
ggsave(paste(results.dir,"compareE0_55.pdf",sep="/"))
...
dflcm = read.csv("C:\\Users\\m_pis\\Box Sync\\Summer Project at Duke (Pistner)\\Master Code Files\\results\\BayesLCM_Full_E1_00_1.csv")[,-1]
dflcm = colMeans(dflcm)

dfgm = read.csv(paste(getwd(),"\\Graphical Models Example\\GM_1_00_1.csv",sep=""),header=FALSE)/samp.size
dfgm = unlist(dfgm)
...
for(i in 1:length(epsilon)){
  ...
  for(q in 1:100){
    ...
    ##PrivBayes
    file.name = paste(getwd(),"\\PrivBayes\\DataSynthesizer\\out\\correlated_attribute_mode\\privBayes_E",a,"_",b,".csv",sep="")
    ...
    file.name = paste(getwd(),"\\Graphical Models Example\\GM_",ep.names[i],"_",q,".csv",sep="")
...
##Creating each model data frames for each epsilon
...
##Creating each model data frames for each epsilon
df1 = as.data.frame(cbind(names.mod1,round(colMeans(LCM.model1[,,3],na.rm=TRUE),3),round(colMeans(privBayes.model1[,,3]),3),round(colMeans(GM.model1[,,3],na.rm=TRUE),3)))
names(df1) = c("names","V1","V2","V3")

df2 = as.data.frame(cbind(names.mod2,round(colMeans(LCM.model2[,,3],na.rm=TRUE),3),round(colMeans(privBayes.model2[,,3]),3),round(colMeans(GM.model2[,,3],na.rm=TRUE),3)))
names(df2) = c("names","V4","V5","V6")
